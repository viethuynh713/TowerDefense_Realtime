...
public class MapGraph
{
    ...
    internal GenerateMapNodeCost FindPathForGenerateMap(Vector2Int src, Vector2Int des, GenerateMapNodeCost cost)
    {
        if (src == des)
        {
            return cost;
    ...
    internal GenerateMapNodeCost FindPathForGenerateBotMap(Vector2Int src, Vector2Int des, GenerateMapNodeCost cost)
    {
        if (src == des)
        {
            return cost;
    ...
    internal List<Vector2Int> DFS(Vector2Int src, Vector2Int des, bool isForGenerateMap)
    {
        GenerateMapNodeCost cost = new GenerateMapNodeCost();
        cost.path.Add(src);
        // Stopwatch stopwatch = new Stopwatch();
        // stopwatch.Start();
        GenerateMapNodeCost resCost = null;
        if (isForGenerateMap)
        {
            resCost = FindPathForGenerateMap(src, des, cost);
        }
        else
        {
            resCost = FindPathForGenerateBotMap(src, des, cost);
        }
        // stopwatch.Stop();
